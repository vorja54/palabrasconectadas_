...
export function getDailyPuzzle(date = new Date(), mode = 'normal') {
  ...
  // Deportes mode: rotating themed sports puzzle (permanent third tab)
  if (mode === 'deportes') {
    return getDeportesPuzzle(date);
  }
...
// Modo DEPORTES: puzzle temático que rota a diario.
// Durante un evento temporal (Mundial), Deportes "se convierte" en el temático de ese evento.
export function getDeportesPuzzle(date = new Date()) {
  // Si hay un evento especial activo (p.ej. Mundial), Deportes muestra ese puzzle
  const mundial = getMundialPuzzle(date);
  if (mundial) return mundial;
  // Rotación diaria normal sobre el pool de 120 puzzles de deportes
  const daysSince = getDaysSince(date);
  const cycle = Math.floor(daysSince / DEPORTES_PUZZLES.length);
  const indexInCycle = daysSince % DEPORTES_PUZZLES.length;
  const shuffledPool = shuffleByCycle(DEPORTES_PUZZLES, cycle);
  return { ...shuffledPool[indexInCycle], date };
}
...
export { PUZZLES, NEW_PUZZLES, SPECIAL_PUZZLES, JASON_PUZZLES, MUNDIAL_PUZZLES, DEPORTES_PUZZLES, getWeeklyPracticePuzzle };
